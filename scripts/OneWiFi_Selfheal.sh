...
onewifi_mem_restart() {
    ...
    if [ "$vmrss" -ge "$threshold2" ]; then
        ...
        echo_t "RSS Memory of Onewifi exceeds RSS threshold2, doing nothing. [Rss : ($vmrss kB) Threshold2 : ($threshold2 kB)]" >> $LOG_FILE
            #systemctl restart onewifi.service
    ...
    #    if [ "$time_since_last_restart" -ge 86400 ]; then
        echo_t "RSS Memory of Onewifi exceeds RSS threshold1, doing nothing [Rss : ($vmrss kB) Threshold1 : ($threshold1 kB)]" >> $LOG_FILE
            #systemctl restart onewifi.service
            #onewifi_last_restart=$now
        #else
